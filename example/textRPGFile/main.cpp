...
void goBattle(_tagPlayer* pPlayer, _tagMonster* pMonster ) {
    ...
    int iArmor = rand() % (pMonster -> iArmorMax - pMonster -> iArmorMin + 1) + (pMonster -> iArmorMin); // 방어값
    ...
    iArmor = rand() % (pPlayer -> iArmorMax - pPlayer -> iArmorMin + 1) + (pPlayer -> iArmorMin); // 방어값
    ...
    if( pPlayer -> iHP <= 0) {
        ...
        // 죽으면 10프로씩 감소시킴
        int iExp = (int)(pPlayer -> iExp * 0.1f);
        int iGold = (int)(pPlayer -> tInventory.iGold * 0.1f);
...
bool loadPlayer(_tagPlayer* pPlayer) {
    ...
    pFile = fopen("player.ply", "rb");

    if(pFile) {

        // 플레이어 이름
        fread(pPlayer->strName, 1, NAME_SIZE, pFile); // 캐릭터형은 1바이트
        // 직업 정보
        fread(&pPlayer->eJob, sizeof(JOB), 1, pFile); // enum 은 4바이트, 배열이 아니므로 1개
        ...
        // 무기 착용여부
        fread(&pPlayer->bEquip[EQ_WEAPON], 1, 1, pFile); // bool 은 1바이트
        // 무기를 착용하였다면 무기 정보도 저장했으므로 가져와야 함
        if(pPlayer->bEquip[EQ_WEAPON]) {
...
bool savePlayer(_tagPlayer* pPlayer) {
    ...
    pFile = fopen("player.ply", "wb");
    if(pFile) {
        // 플레이어 이름
        fwrite(pPlayer->strName, 1, NAME_SIZE, pFile); // 캐릭터형은 1바이트
        // 직업 정보
        fwrite(&pPlayer->eJob, sizeof(JOB), 1, pFile); // enum 은 4바이트, 배열이 아니므로 1개
        ...
        // 무기 착용여부
        fwrite(&pPlayer->bEquip[EQ_WEAPON], 1, 1, pFile); // bool 은 1바이트
        // 무기를 착용하였다면 무기 정보도 저장했으므로 가져와야 함
        if(pPlayer->bEquip[EQ_WEAPON]) {
...
void setStore(_tagItem* pWeaponStore, _tagItem* pArmorStore) {

    FILE* pFile = NULL;
    pFile = fopen("Store.str", "rb");
    if(pFile) {

        fread(pWeaponStore, sizeof(_tagItem), STORE_WEAPON_MAX, pFile);
        fread(pArmorStore, sizeof(_tagItem), STORE_ARMOR_MAX, pFile);
